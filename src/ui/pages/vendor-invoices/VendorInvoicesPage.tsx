...
export default function VendorInvoicesPage() {
  ...
  const [editing] = useState<VendorInvoice | null>(null)
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      await Promise.all(result.data.map(async (inv: VendorInvoice) => {
        try {
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Purchase Invoice' : 'Add Purchase Invoice'}>
        <div className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <button onClick={handleSubmit} className="px-4 py-2 text-sm rounded-xl bg-[#6B7280] text-white font-medium hover:opacity-90">Save</button>
          </div>
        </div>
      </Modal>
