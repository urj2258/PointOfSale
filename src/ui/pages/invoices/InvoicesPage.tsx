...
import type { Customer, InventoryItem, Invoice, PaginatedResult } from '../../types'
...
export default function InvoicesPage() {
  ...
  const handleSubmit = async () => {
    ...
    if (editing) {
      await api.invoices.update(editing.id, form.customer_id, form.invoice_number, form.issue_date, form.due_date,
      ...
      await api.invoices.create(form.customer_id, form.invoice_number, form.issue_date, form.due_date,
  ...
  const handleMarkPaid = async (inv: Invoice) => {
    if (confirm(`Mark invoice "${inv.invoice_number}" as paid?`)) {
      await api.invoices.markPaid(inv.id)
      load()
    }
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (inv: Invoice) => `$${inv.remaining_balance.toLocaleString()}`,
    },
    {
