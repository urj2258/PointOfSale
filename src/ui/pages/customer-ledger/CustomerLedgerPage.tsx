...
export default function CustomerLedgerPage() {
  ...
  const load = useCallback(async () => {
    ...
    const result = await api.customerLedger.list(
      filterCustomer || undefined, filterDateFrom || undefined, filterDateTo || undefined, page, 20
    )
    setData(result)
    setLoading(false)
  ...
  const openCreate = () => {
    ...
    setEditing(null)
    setForm({ customer_id: '', product_id: '', transaction_datetime: now, quantity: 0, rate_per_unit: 0, total_payment: 0, paid_amount: 0, description: '', vehicle_number: '' })
    setError('')
    setModalOpen(true)
  }
  ...
  const handleSubmit = async () => {
    ...
    setError('')
    if (editing) {
      await api.customerLedger.update(editing.id, form.paid_amount, form.description || undefined, form.vehicle_number || undefined)
    } else {
      await api.customerLedger.create(form.customer_id, form.product_id, form.transaction_datetime,
  ...
  const handleDelete = async (entry: CustomerLedgerEntry) => {
    if (confirm('Delete this sale entry?')) {
      await api.customerLedger.delete(entry.id)
      load()
    }
  }
    ...
    <div className="space-y-6">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editing ? 'Edit Sale' : 'New Sale'}>
        <div className="space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          {editing && (
            <>
              <p className="text-sm text-brand-text-muted">Editing payment for sale entry only.</p>
              <div>
                <label className="block text-sm font-medium text-brand-text-primary dark:text-white mb-1">Paid Amount *</label>
                <input type="number" value={form.paid_amount} onChange={(e) => setForm({ ...form, paid_amount: Number(e.target.value) })}
                  className="w-full px-3 py-2 rounded-xl border border-white/30 dark:border-white/[0.1] bg-white/40 dark:bg-white/[0.04] text-sm text-brand-text-primary dark:text-white focus:outline-none focus:ring-2 focus:ring-brand-primary/40" />
